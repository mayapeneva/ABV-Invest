...
namespace ABV_Invest.Web.Areas.Identity.Pages.Account.Manage
{
    ...
    public class EnableAuthenticatorModel : PageModel
    {
        ...
        {
            [Required]
            [StringLength(7, ErrorMessage = "Кодът за потвърждение трябва да е поне {2} и не повече от {1} символа дълъг.", MinimumLength = 6)]
        ...
        private async Task LoadSharedKeyAndQrCodeUriAsync(AbvInvestUser user)
        {
        ...
        private string GenerateQrCodeUri(string email, string unformattedKey)
        {
            return string.Format(
                AuthenticatorUriFormat, this._urlEncoder.Encode("ABV_Invest.Web"), this._urlEncoder.Encode(email),
                unformattedKey);
        }
    }
}